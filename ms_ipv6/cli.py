...
def create_parser() -> argparse.ArgumentParser:
    ...
    dl_parser.add_argument("--local-dir", required=True, help="文件保存的本地根目录（必填）")
    dl_parser.add_argument("--workers", type=int, default=4, help="并发下载线程数，默认 4")
    ...
    dl_parser.add_argument("--only-raw", action="store_true", help="仅下载带 raw_url 的条目（IPv6 直链）")
    dl_parser.add_argument("--only-no-raw", action="store_true", help="仅下载不带 raw_url 的条目（回源地址）")
    dl_parser.add_argument("--timeout", type=int, default=60, help="HTTP 超时秒数，默认 60")
